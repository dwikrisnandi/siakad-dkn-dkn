...
export default function DosenUjian() {
	...
	useEffect(() => {
		api
			.get("/schedules")
			.then((r) => setSchedules(r.data.filter((s) => s.dosen_id === user.id)))
			.catch(() => {});
	}, [user.id]);
	...
	const fetchExams = async () => {
		setLoading(true);
		try {
			const r = await api.get(`/exams/schedule/${selectedSchedule}`);
			setExams(r.data);
		} catch (e) {
			console.error(e);
		} finally {
	...
	const handleToggleBlock = async (student) => {
		try {
			...
			await api.post(`/exams/${activeExam.id}/blocks`, {
				mahasiswa_id: student.id,
				is_blocked: newBlocked,
			});
			setStudents((prev) =>
				prev.map((s) =>
					s.id === student.id ? { ...s, is_blocked: newBlocked } : s,
				),
			);
		} catch (e) {
	...
	const handleSaveEssay = async (answerId) => {
		...
		try {
			await api.put(`/exam-answers/${answerId}/score`, {
				essay_score: essayScores[answerId],
			});
			...
			setSavingEssay((p) => ({ ...p, [answerId]: false }));
		}
	};
	...
	const openBankModal = async () => {
		try {
			const r = await api.get(
				`/question-bank/schedule/${activeExam.schedule_id}`,
			);
			setBankQuestions(r.data);
			const currentBankIds = questions
				.filter((q) => q.bank_id)
				.map((q) => q.bank_id);
			setSelectedBankIds(currentBankIds);
			setShowBankModal(true);
		} catch {
			alert("Gagal memuat daftar soal bank");
		}
	};

	const handleDownloadDocx = () => {
		window.open(
			`/api/exams/${activeExam.id}/export-docx?token=${localStorage.getItem("token")}`,
			"_blank",
		);
	};
	...
	const handleGenerateKisi = async () => {
		...
		try {
			const res = await api.post("/ai-generate-kisi", {
				exam_id: activeExam.id,
			});
			setActiveExam({ ...activeExam, kisi_kisi: res.data.kisi_kisi });
			const ex = exams.map((e) =>
				e.id === activeExam.id ? { ...e, kisi_kisi: res.data.kisi_kisi } : e,
			);
			setExams(ex);
	...
	const handleSyncGrades = async (examOverride) => {
		...
		if (
			!window.confirm(
				`Kirim semua nilai ${targetExam.type} ini ke Modul Nilai Akhir? Nilai yang ada di Modul Nilai Akhir akan ditimpa dengan hasil ini.`,
			)
		)
			return;
		try {
	...
	const handleImportBank = async () => {
		...
		try {
			await api.post(`/exams/${activeExam.id}/import-from-bank`, {
	...
	const openEditExam = (exam) => {
		...
		setExamForm({
			...
			start_time: toLocal(exam.start_time),
			end_time: toLocal(exam.end_time),
			...
			<div className="animate-fade-in">
				<div className="d-flex align-items-center gap-3 mb-4">
					...
					<div className="flex-grow-1">
						<h3 className="fw-bold mb-0">Peserta & Hasil Ujian</h3>
						<small className="text-muted">{activeExam?.title}</small>
					</div>
					<button
						className="btn btn-success fw-bold d-flex align-items-center gap-2 shadow-sm"
						...
						<table className="table table-hover align-middle mb-0">
							...
							<tbody>
								{students.length === 0 ? (
									...
									students.map((s) => {
										const session = results?.sessions?.find(
											(res) => res.mahasiswa_id === s.id,
										);
										let essayAnswers = [];
										let essayGraded = 0;
										if (session) {
											essayAnswers = session.answers.filter(
												(a) => a.question_type === "essay",
											);
											essayGraded = essayAnswers.filter(
												(a) => a.graded_by_dosen,
											).length;
										}
																...
																<>
																	<button
																		...
																		onClick={async () => {
																			if (
																				!window.confirm(
																					"Yakin ingin membuka kembali ujian untuk mahasiswa ini? Waktu akan berlanjut dan nilai sementara akan di-reset.",
																				)
																			)
																				return;
																			try {
																				await api.post(
																					`/exam-sessions/${session.id}/reopen`,
																				);
																	...
																	>
																		{openDropdowns[s.id]
																			? "Tutup Jawaban"
																			: `Koreksi Essay (${essayGraded}/${essayAnswers.length})`}
																	</button>
																</>
															)}
														</div>
															...
															<div className="bg-light px-4 py-4 border-bottom shadow-inner">
																{essayAnswers.length === 0 ? (
																	<div className="text-center text-muted small py-3">
																		Tidak ada soal essay untuk mahasiswa ini.
																	</div>
																) : (
																	essayAnswers.map((a, i) => (
																		<div
																			key={a.id}
																			className="p-3 rounded-3 mb-3 border bg-white shadow-sm"
																		>
																			...
																			<div className="d-flex flex-column gap-2">
																				...
																				<div className="d-flex align-items-center gap-2 flex-wrap mt-2">
																					<button
																						...
																						onClick={() => handleAIGrade(a)}
																						disabled={gradingAI[a.id]}
																					>
																						{gradingAI[a.id] ? (
																					...
																					<div className="ms-auto d-flex align-items-center gap-2 bg-white border p-1 rounded-3">
																						...
																						<button
																							className="btn btn-sm btn-success ms-2 rounded-2"
																							onClick={() =>
																								handleSaveEssay(a.id)
																							}
																							disabled={savingEssay[a.id]}
																						>
																							{savingEssay[a.id]
																								? "..."
																								: "Simpan"}
																				...
																				{aiFeedbacks[a.id] && (
																					<div className="w-100 mt-2 p-2 rounded bg-primary bg-opacity-10 border border-primary border-opacity-25 text-primary small fst-italic">
																						🤖 AI Feedback: {aiFeedbacks[a.id]}
																					</div>
																				)}
																			</div>
																		</div>
				...
				<div className="d-flex align-items-center gap-3 mb-4 flex-wrap">
					...
					<div className="flex-grow-1">
						...
						<small className="text-muted">
							{activeExam?.title} — {activeExam?.type}
						</small>
					</div>
								...
								<div className="d-flex justify-content-between align-items-start gap-3">
									<div className="flex-grow-1">
										<div className="d-flex gap-2 align-items-center mb-2">
											...
											<span
												className={`badge bg-${typeBadge[q.question_type]}`}
											>
											...
											<div className="row g-1">
												{q.options.map((opt, oi) => (
													<div key={oi} className="col-6">
														<span
															className={`d-block small px-2 py-1 rounded ${String.fromCharCode(65 + oi) === q.correct_answer ? "bg-success text-white fw-bold" : "text-muted"}`}
														>
								...
								<div className="modal-content border-0 shadow">
									<div className="modal-header border-0 pb-0">
										...
										<button
											type="button"
											className="btn-close"
											onClick={() => !importingBank && setShowBankModal(false)}
										></button>
									</div>
									<div className="modal-body">
										{bankQuestions.length === 0 ? (
											<div className="text-center text-muted py-5">
												<h5>
													Belum ada soal di Bank Soal untuk matakuliah ini
												</h5>
											</div>
										) : (
											<div className="row g-2">
												<div className="col-12 mb-2 d-flex justify-content-between align-items-center">
													<span className="text-muted fw-bold">
														{selectedBankIds.length} / {bankQuestions.length}{" "}
														Terpilih
													</span>
													<button
														className="btn btn-sm btn-outline-secondary"
									...
									<div className="modal-footer border-0 pt-0 mt-3">
										...
										>
											{importingBank
												? "Menyimpan..."
												: `Simpan ${selectedBankIds.length} Soal`}
										</button>
									</div>
								</div>
							</div>
						...
						>
							<div className="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
								<div className="modal-content border-0 shadow">
									<div className="modal-header border-0 pb-0">
										<h5 className="modal-title fw-bold">
											Kisi-Kisi Ujian: {activeExam?.title}
										</h5>
										<button
											type="button"
											className="btn-close"
									...
									<div className="modal-body">
										{activeExam?.kisi_kisi ? (
											...
											<div className="text-center text-muted py-4">
												...
												<p className="small">
													Klik tombol di bawah untuk generate otomatis dengan
													AI.
												</p>
											</div>
										)}
	...
	return (
		<div className="animate-fade-in">
					...
					>
						...
						{schedules.map((s) => (
							<option key={s.id} value={s.id}>
								{s.course_code} - {s.course_name} ({s.class_name})
							</option>
						))}
					</select>
				</div>
			...
			{selectedSchedule &&
				(loading ? (
					<div className="text-center text-muted py-4">Memuat...</div>
				) : exams.length === 0 ? (
					<div className="text-center text-muted py-5">
						<ClipboardList size={48} className="mb-3 opacity-50" />
								...
								<div className="card shadow-sm border-0 rounded-4 h-100">
									<div className="card-body p-4">
										<div className="d-flex justify-content-between align-items-start mb-3">
											<div>
												<span
													className={`badge ${exam.type === "UTS" ? "bg-warning text-dark" : "bg-danger"} mb-2`}
												>
													{exam.type}
												</span>
											...
											<span
												className={`badge ${exam.is_active ? "bg-success" : "bg-secondary"}`}
											>
										...
										<div className="d-flex flex-wrap gap-3 text-muted small mb-4">
											...
											>
												{exam.total_questions === 0
													? "⚠️ 0 Soal (Kosong)"
													: `📝 ${exam.total_questions} Soal Tersedia`}
											</span>
										...
										<div className="d-flex flex-wrap gap-2">
											...
											<button
												className={`btn btn-sm ${exam.is_active ? "btn-success" : "btn-outline-secondary"}`}
						...
						<div className="modal-dialog modal-dialog-centered">
							<div className="modal-content border-0 shadow">
								<div className="modal-header border-0">
									<h5 className="modal-title fw-bold">
										{editExamId ? "Edit Ujian" : "Buat Ujian Baru"}
									</h5>
									<button
										className="btn-close"
