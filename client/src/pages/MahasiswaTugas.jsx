...
async function imageHandler(quillRef) {
  ...
  input.onchange = async () => {
        ...
        reader.onload = (event) => {
          ...
          img.onload = () => {
            ...
            canvas.toBlob((blob) => {
              ...
              const compressedFile = new File([blob], fileSource.name.replace(/\.[^/.]+$/, "") + ".jpg", {
                type: 'image/jpeg',
    ...
    try {
      ...
      alert('Gagal mengupload gambar. Pastikan format didukung dan tidak melebihi batas sistem.');
      console.error(err);
    }
  };
}
...
export default function MahasiswaTugas() {
    ...
    try {
      ...
      for (const a of assignData) {
        try {
          ...
          const mySub = subRes.data.find(s => Number(s.mahasiswa_id) === Number(user.id));
  ...
  useEffect(() => { loadAssignmentsAndSubmissions(); }, [selectedSchedule, user.id]);

  const handleSubmitTugas = async (e) => {
    e.preventDefault();
    if (!submittingFor) return;
    const isEmpty = !answerText || answerText === '<p><br></p>' || answerText.trim() === '';
    if (isEmpty) { alert('Jawaban tidak boleh kosong.'); return; }
    ...
    try {
      ...
      setSubmitResult({ success: true, message: 'Jawaban berhasil dikumpulkan!' });
      setAnswerText('');
  ...
  const openSubmit = (assignment) => {
    ...
    setAnswerText(existing?.file_data && (existing.file_data.startsWith('<') || existing.file_data.includes('</'))
      ? existing.file_data : '');
    ...
    <div className="animate-fade-in">
      ...
      {!selectedSchedule ? (
        <>
          ...
          {schedules.length === 0 ? (
            <div className="text-center text-muted py-5"><PenTool size={48} className="mb-3 opacity-50" /><h5>Belum ada jadwal</h5></div>
          ) : (
                  ...
                  <div className="card shadow-sm border-0 rounded-4 h-100" style={{ cursor: 'pointer', transition: 'transform 0.15s' }}
                    ...
                    <div className="card-body p-4">
                      <div className="d-flex align-items-center mb-3">
                        <div className="bg-warning-subtle p-3 rounded-3 me-3"><PenTool size={22} className="text-warning" /></div>
                      ...
                      <div className="d-flex gap-2 text-muted" style={{ fontSize: '12px' }}>
                        <span>🎓 {s.dosen_name}</span><span>•</span><span>📅 {s.day}</span>
                      </div>
                    </div>
        ...
        <div className="row g-4">
          {loading ? (
            ...
            <div className="col-12 text-center text-muted py-5"><PenTool size={48} className="mb-3 opacity-50" /><h5>Belum ada tugas</h5></div>
          ) : (
            assignments.map((a) => {
                  ...
                  <div className={`card shadow-sm border-0 h-100 rounded-4 border-top border-3 border-${isSubmitted ? 'primary' : status.color}`}>
                    <div className="card-body p-4 pb-2">
                      <div className="d-flex justify-content-between align-items-start mb-2">
                        ...
                        {isSubmitted
                          ? <span className="badge bg-primary-subtle text-primary border border-primary-subtle"><CheckCircle size={12} className="me-1 mb-1" /> Dikumpulkan</span>
                          : <span className={`badge bg-${status.color}-subtle text-${status.color} border border-${status.color}-subtle`}>{status.label}</span>
                        }
                      </div>
                    ...
                    <div className="card-footer bg-white border-0 p-3 pt-0 rounded-bottom-4">
                      {isSubmitted ? (
                        <div className="d-flex gap-2">
                          {status.active && (
                            <>
                              <button onClick={() => openSubmit(a)} className="btn btn-outline-primary flex-grow-1 btn-sm fw-bold"><UploadCloud size={14} className="me-1" />Edit</button>
                              <button onClick={() => handleDeleteTugas(a.id)} className="btn btn-outline-danger flex-grow-1 btn-sm fw-bold"><Trash2 size={14} className="me-1" />Hapus</button>
                            </>
                          )}
                          {!status.active && <button className="btn btn-secondary w-100 btn-sm fw-bold" disabled>Selesai (Ditutup)</button>}
                        </div>
                      ) : (
                        status.active ? (
                          <button onClick={() => openSubmit(a)} className="btn btn-primary w-100 btn-sm fw-bold d-flex justify-content-center align-items-center gap-2">
            ...
            <div className="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
              <div className="modal-content border-0 shadow">
                <div className="modal-header border-0 pb-0">
                  <div>
                    <h5 className="modal-title fw-bold mb-0">{mySubmissions[submittingFor.id] ? 'Edit Jawaban' : 'Kumpulkan Tugas'}</h5>
                    <small className="text-muted">{submittingFor.title}</small>
                  </div>
                  <button type="button" className="btn-close" onClick={() => setSubmittingFor(null)} />
                </div>
                <div className="modal-body">
                  {submitResult ? (
                    <div className="text-center py-4">
                      {submitResult.success ? <CheckCircle size={56} className="text-success mb-3" /> : <XCircle size={56} className="text-danger mb-3" />}
                    ...
                    <form onSubmit={handleSubmitTugas}>
                      ...
                      <div className="bg-light p-3 rounded-3 mb-3 border">
                        ...
                        <div className="text-dark small ql-snow">
                          <div className="ql-editor p-0" style={{ minHeight: 'unset' }} dangerouslySetInnerHTML={{ __html: submittingFor.description }} />
                        </div>
                      </div>
                      ...
                      <div className="alert alert-info py-2 small mb-3">
                        💡 <b>Tips:</b> Kamu bisa mengetik jawaban langsung di editor ini. Gunakan ikon <b>🖼️ gambar</b> di toolbar untuk menyisipkan <b>screenshot kode</b> secara inline.
                      </div>

                      <label className="form-label text-muted small fw-bold">Jawaban / Penjelasan</label>
                      <div className="border rounded-3 overflow-hidden mb-3" style={{ minHeight: '250px' }}>
                        <ReactQuill
                          ref={quillRef}
                          theme="snow"
                      ...
                      <div className="d-flex gap-2 justify-content-end mt-2">
                        ...
                        <button type="submit" className="btn btn-primary px-4" disabled={submitting}>
                          {submitting ? 'Menyimpan...' : (mySubmissions[submittingFor.id] ? 'Perbarui Jawaban' : '📤 Kumpulkan')}
                        </button>
                      </div>
                    </form>
                  )}
