...
import { PenTool, Plus, Users, FileText, XCircle, CheckCircle, Eye, ArrowLeft, BrainCircuit } from 'lucide-react';
...
export default function DosenTugas() {
  ...
  const openEditModal = (a) => {
    setEditMode(true); setEditingId(a.id);
    let isoStr = '';
    try { const d = new Date(a.deadline); isoStr = new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 16); } catch (e) {}
    setFormData({ title: a.title, description: a.description, deadline: isoStr });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode && editingId) {
        ...
        await api.post('/assignments', { schedule_id: parseInt(selectedSchedule), ...formData });
      }
      setShowModal(false);
      setFormData({ title: '', description: '', deadline: '' });
      const res = await api.get(`/assignments/${selectedSchedule}`);
  ...
  const handleViewSubmissions = async (assignment) => {
    ...
    try {
      ...
      const initialGrades = {};
      res.data.forEach(s => { initialGrades[s.id] = s.nilai || ''; });
  ...
  useEffect(() => {
    if (assignments.length > 0 && location.state?.assignment_id) {
      const targetA = assignments.find(a => a.id === location.state.assignment_id);
  ...
  const handleViewFile = (submission) => {
    ...
    if (!data) { alert('Belum ada file/jawaban.'); return; }

    if (data.startsWith('<') || data.includes('</')) {
      const win = window.open('', '_blank');
      win.document.write(`<!DOCTYPE html><html><head><title>Jawaban Tugas - ${submission.mahasiswa_name}</title><style>body{font-family:sans-serif;padding:32px;max-width:860px;margin:auto;line-height:1.6} img{max-width:100%;border-radius:8px;margin:8px 0} code{background:#f4f4f4;padding:2px 6px;border-radius:4px} pre{background:#f4f4f4;padding:16px;border-radius:8px;overflow-x:auto}</style></head><body>${data}</body></html>`);
      return;
    }

    if (!data.startsWith('data:')) {
      alert('Format jawaban tidak dikenali.'); return;
    }
  ...
  const handleAiGrade = async (sub) => {
    ...
    try {
      ...
      const isHtml = sub.file_data && (sub.file_data.startsWith('<') || sub.file_data.includes('</'));
      ...
      const payload = {
        ...
        submission_text: isHtml ? sub.file_data : (!isImage ? sub.file_data : null),
      ...
      setAiGrading(prev => ({ ...prev, [sub.id]: { loading: false, error: msg } }));
    }
  };
            ...
            <div className="row g-3">
              {schedules.map(s => (
                <div className="col-md-6 col-lg-4" key={s.id}>
                  <div className="card shadow-sm border-0 rounded-4 h-100" style={{ cursor: 'pointer', transition: 'transform 0.15s' }}
                    onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-4px)'}
                    onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}
                    onClick={() => setSelectedSchedule(s.id)}>
                    <div className="card-body p-4">
                      <div className="d-flex align-items-center mb-3">
                        <div className="bg-primary-subtle p-3 rounded-3 me-3"><Users size={22} className="text-primary" /></div>
                      ...
                      <div className="d-flex gap-2 text-muted" style={{ fontSize: '12px' }}><span>📅 {s.day}</span></div>
                    </div>
        ...
        <div className="row g-4">
          {loading ? (
            ...
            <div className="col-12 text-center text-muted py-5">
              <PenTool size={48} className="mb-3 opacity-50" /><h5>Belum ada tugas</h5>
            </div>
          ) : (
            assignments.map((a) => (
                ...
                <div className="card shadow-sm border-0 h-100 rounded-4">
                  <div className="card-body p-4 pb-0">
                    <div className="d-flex justify-content-between align-items-start mb-2">
                      ...
                      {new Date(a.deadline) < new Date() ? <span className="badge bg-danger">Ditutup</span> : <span className="badge bg-success">Aktif</span>}
                    </div>
                    ...
                    <div className="text-muted small mb-4 ql-snow"><div className="ql-editor p-0" style={{minHeight: 'unset'}} dangerouslySetInnerHTML={{ __html: a.description }} /></div>
                  </div>
              ...
              <div className="modal-content border-0 shadow">
                ...
                <div className="modal-body">
                  <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-3">
                      <label className="form-label text-muted small fw-bold">Instruksi / Penjelasan Tugas</label>
                      <div className="border rounded-3 overflow-hidden" style={{ minHeight: '200px' }}>
                        <ReactQuill
                          theme="snow"
                    ...
                    <div className="mb-3">
                      ...
                      <input type="datetime-local" className="form-control" required value={formData.deadline} onChange={e => setFormData({ ...formData, deadline: e.target.value })} />
                    </div>
            ...
            <div className="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
              <div className="modal-content border-0 shadow">
                <div className="modal-header border-bottom-0 pb-0">
                  ...
                  <button type="button" className="btn-close" onClick={() => setViewingAssignment(null)} />
                </div>
                <div className="modal-body">
                  {loadingSubmissions ? (
                    ...
                    <div className="text-center text-muted py-4">
                      <FileText size={48} className="mb-3 opacity-50" /><h5>Belum Ada yang Mengumpulkan</h5>
                    </div>
                  ) : (
                    <div className="table-responsive">
                      <table className="table table-hover align-middle">
                        <thead className="table-light">
                          <tr>
                            <th>NIM</th><th>Nama</th><th>Waktu Kumpul</th><th>File/Jawaban</th><th>Nilai</th><th>Koreksi AI</th>
                          </tr>
                        </thead>
                        <tbody>
                          {submissions.map((s) => {
                            ...
                            return (
                              <React.Fragment key={s.id}>
                                <tr>
                                  ...
                                  <td>
                                    {s.file_data ? (
                                      <button className="btn btn-sm btn-outline-primary d-flex align-items-center gap-1" onClick={() => handleViewFile(s)}>
                                        <Eye size={14} /> Lihat
                                      </button>
                                  ...
                                  <td>
                                    <div className="d-flex align-items-center gap-2">
                                      <input type="number" className="form-control form-control-sm text-center" style={{ width: '70px' }}
                                        ...
                                        onChange={(e) => setGradingValues(prev => ({ ...prev, [s.id]: e.target.value }))}
                                      />
                                      <button className="btn btn-sm btn-success fw-bold" onClick={() => handleSaveGrade(s.id)} title="Simpan Nilai">
                                  ...
                                  <td>
                                    <button
                                      ...
                                      style={{ borderColor: '#7c3aed', color: '#7c3aed', whiteSpace: 'nowrap' }}
                                      onClick={() => handleAiGrade(s)}
                                      disabled={ai?.loading}
                                    >
                                      <BrainCircuit size={14} />
                                      {ai?.loading ? 'Menganalisis...' : '🤖 Koreksi AI'}
                                    </button>
                                  </td>
                                </tr>
                                {/* AI Result row */}
                                {ai && !ai.loading && (
                                  <tr style={{ background: ai.error ? '#fff5f5' : ai.ai_terindikasi ? '#fffbeb' : '#f0fdf4' }}>
                                    <td colSpan={6}>
                                      {ai.error ? (
                                        <div className="text-danger small"><XCircle size={14} className="me-1" />{ai.error}</div>
                                      ) : (
                                        <div className="p-2">
                                          <div className="d-flex align-items-start gap-3 mb-2">
                                            <div className="text-center">
                                              <div className="fw-bold" style={{ fontSize: '28px', color: ai.ai_terindikasi ? '#d97706' : '#16a34a' }}>{ai.skor}</div>
                                              <small className="text-muted">Skor AI</small>
                                            </div>
                                            <div className="flex-grow-1">
                                              <p className="mb-1 small fw-bold text-success">💡 Feedback:</p>
                                              <p className="mb-2 small">{ai.feedback}</p>
                                            </div>
                                            <button
                                              className="btn btn-sm btn-success"
                                              onClick={async () => {
                                                setGradingValues(prev => ({ ...prev, [s.id]: ai.skor }));
                                                try {
                                                  // Save AI Feedback to JSON immediately as requested
                                                  await api.post(`/submissions/${s.id}/ai-feedback`, { feedback: ai.feedback, skor: ai.skor });
                                                  alert('Feedback AI telah diteruskan ke mahasiswa.');
                                                } catch (e) {
                                          ...
                                          {/* AI Detection Badge */}
                                          <div className={`rounded-3 px-3 py-2 small ${ai.ai_terindikasi ? 'bg-warning-subtle border border-warning-subtle text-warning-emphasis' : 'bg-success-subtle border border-success-subtle text-success-emphasis'}`}>
                                            <span className="fw-bold me-2">{ai.ai_terindikasi ? '🤖 Terindikasi Jawaban AI' : '✅ Jawaban Asli Mahasiswa'}</span>
