...
export default function AdminTahunAkademik() {
  ...
  const handleActivate = async (id) => {
    if (!window.confirm('Aktifkan Tahun Akademik ini? Jadwal semester lain akan disembunyikan.')) return;
    try {
              ...
              <tbody>
                {years.map(y => (
                  <tr key={y.id} className={y.is_active ? 'table-primary' : ''}>
                    ...
                    <td>
                      {y.is_active ? (
                        <span className="badge bg-success"><CheckCircle size={12} className="me-1 mb-1"/> Aktif</span>
                      ) : (
                    ...
                    <td className="pe-4 text-end">
                      {!y.is_active && (
                        <button className="btn btn-sm btn-outline-success me-2" onClick={() => handleActivate(y.id)}>
                          Aktifkan
                        </button>
                      )}
                      {!y.is_active && (
                        <button className="btn btn-sm btn-outline-danger" onClick={() => handleDelete(y.id)}>
                          <Trash2 size={16} />
                        </button>
                      )}
                    </td>
                ...
                {years.length === 0 && (
                  <tr><td colSpan="3" className="text-center py-4 text-muted">Belum ada data Tahun Akademik</td></tr>
                )}
              </tbody>
            </table>
          )}
            ...
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content border-0 shadow">
                ...
                <div className="modal-footer border-0 pt-0">
                  ...
                  <button type="submit" form="yearForm" className="btn btn-primary fw-bold">Simpan</button>
                </div>
              </div>
            </div>
          </div>
