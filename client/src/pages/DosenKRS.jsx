...
export default function DosenKRS() {
	...
	const handleAction = async (action) => {
		if (
			!window.confirm(
				`Yakin ingin ${action === "Approved" ? "MENYETUJUI" : "MENOLAK"} KRS ini?`,
			)
		)
			return;
		...
		try {
			...
			setKrsData((prev) => ({
				...prev,
				krs: { ...prev.krs, status_approval: action },
			}));
			setStudents((prev) =>
				prev.map((s) =>
					s.id === selectedStudent.id ? { ...s, krs_status: action } : s,
				),
			);
		} catch (err) {
		...
		<div className="animate-fade-in">
			<div className="mb-4">
				<h3 className="fw-bold mb-0 text-dark">
					<Users size={28} className="me-2 text-primary" /> Bimbingan Akademik
					(DPA)
				</h3>
				<p className="text-muted small mb-0">
					Kelola persetujuan KRS untuk anak wali Anda
				</p>
			</div>
			...
			{selectedStudent ? (
				<div className="card border-0 shadow-sm rounded-4 overflow-hidden mb-5 animate-fade-in">
					<div className="card-header bg-white border-bottom p-4 d-flex justify-content-between align-items-center">
						<div>
							<button
								className="btn btn-sm btn-light text-primary fw-bold mb-2 rounded-pill px-3"
								onClick={closeStudentKRS}
							>
								&larr; Kembali ke Daftar
							</button>
							<h5 className="fw-bold mb-0">{selectedStudent.name}</h5>
							<div className="text-muted small">
								NIM: {selectedStudent.nidn_nim} | Prodi:{" "}
								{selectedStudent.nama_prodi}
							</div>
						</div>
							...
							<div>
								{krsData.krs.status_approval === "Pending" && (
									<span className="badge bg-warning text-dark border">
										<Clock size={14} className="me-1" /> Menunggu Review
									</span>
								)}
								{krsData.krs.status_approval === "Approved" && (
									<span className="badge bg-success border">
										<CheckCircle size={14} className="me-1" /> Disetujui
									</span>
								)}
								{krsData.krs.status_approval === "Rejected" && (
									<span className="badge bg-danger border">
										<XCircle size={14} className="me-1" /> Ditolak
									</span>
								)}
							</div>
						) : (
					...
					<div className="card-body p-0">
						{loadingKrs ? (
							...
							<div className="text-center py-5 text-muted">
								Mahasiswa ini belum mengajukan KRS.
							</div>
						) : (
							<>
									...
									<table className="table table-hover align-middle mb-0">
										...
										<tfoot className="table-light fw-bold">
											<tr>
												...
												<td colSpan="4">
													{krsData.items.reduce((s, i) => s + i.sks, 0)} SKS
												</td>
											</tr>
										</tfoot>
									</table>
								...
								{loading ? (
									<tr>
										<td colSpan="5" className="text-center py-4">
											Memuat data anak wali...
										</td>
									</tr>
								) : filtered.length === 0 ? (
									<tr>
										<td colSpan="5" className="text-center py-4 text-muted">
											Belum ada mahasiswa anak wali.
										</td>
									</tr>
								) : (
									filtered.map((s) => (
										<tr key={s.id}>
											...
											<td>{s.nama_prodi || "-"}</td>
											<td>
												{s.krs_status === "Approved" && (
													<span className="badge bg-success-subtle text-success border border-success-subtle">
														<CheckCircle size={12} className="me-1" /> Disetujui
													</span>
												)}
												{s.krs_status === "Pending" && (
													<span className="badge bg-warning-subtle text-warning border border-warning-subtle">
														<Clock size={12} className="me-1" /> Perlu Review
													</span>
												)}
												{s.krs_status === "Rejected" && (
													<span className="badge bg-danger-subtle text-danger border border-danger-subtle">
														<XCircle size={12} className="me-1" /> Ditolak
													</span>
												)}
												{!s.krs_status && (
