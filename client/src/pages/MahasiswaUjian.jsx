...
export default function MahasiswaUjian() {
	...
	const setPendingQueue = (examId, queue) => {
		try {
			localStorage.setItem(
				LS_KEY_PREFIX + examId + "_pending",
				JSON.stringify(queue),
			);
		} catch (e) {
			console.error("localStorage pending save error:", e);
		}
	};
	...
	const syncPendingAnswers = useCallback(
		async (examId) => {
			...
			for (const item of queue) {
				try {
					await api.post(`/exam-sessions/${eid}/answer`, {
	...
	const syncDeferredSubmissions = async () => {
		for (let i = 0; i < localStorage.length; i++) {
			...
			try {
				...
				try {
					await api.post(`/exam-sessions/${eid}/start`, {
				...
				for (const [qid, ans] of Object.entries(answers)) {
					try {
						...
						console.error("Deferred sync error for q", qid, e);
					}
				}
			...
			if (offlineMode && activeExam?.id) {
				(async () => {
					try {
						...
						await api.post(`/exam-sessions/${activeExam.id}/start`, {
							token: cachedExam?.token || "",
						});
						// Sync all localStorage answers as pending
						const localAnswers = getLocalAnswers(activeExam.id);
						const allPending = Object.entries(localAnswers).map(
							([qid, ans]) => ({ question_id: parseInt(qid), answer: ans }),
						);
			...
			try {
				...
				if (detailRes.data && Array.isArray(detailRes.data.questions)) {
					await cacheExamData({
						...
						token: exam.token || "",
						duration_minutes:
							exam.duration_minutes || detailRes.data.duration_minutes || 90,
						course_name: exam.course_name || "",
						course_code: exam.course_code || "",
					});
					console.log(`✅ Pre-cached exam: ${exam.title} (ID: ${exam.id})`);
					try {
						await api.post(`/exams/${exam.id}/cache-status`);
					} catch (e) {}
				}
			} catch (e) {
	...
	const shuffleQuestions = (questions, examId) => {
		...
		for (let i = 0; i < shuffled.length; i++) {
			...
			if (q.question_type === "pg" && Array.isArray(q.options)) {
				const opts = q.options.map((optText, idx) => ({
	...
	const handleStartExam = async (e) => {
		...
		if (!navigator.onLine) {
			try {
				...
				if (!cachedExam) {
					setTokenError(
						"Soal ujian belum ter-cache di perangkat ini. Hubungkan internet untuk mengunduh soal.",
					);
					setIsStarting(false);
					return;
				}
				...
				// Start timer
				const durationSec =
					(targetExam.duration_minutes || cachedExam.duration_minutes || 90) *
					60;
					...
					if (pollCounter % 10 === 0 && navigator.onLine) {
						api
							...
							.then((res) => {
								if (res.data.blocked) {
									alert(
										"Ujian dihentikan karena diblokir oleh Dosen atau terdeteksi pelanggaran. Silakan hubungi dosen.",
									);
									clearInterval(timerRef.current);
									if (document.exitFullscreen && document.fullscreenElement) {
										document.exitFullscreen().catch((e) => console.log(e));
		...
		try {
			...
			const startRes = await api.post(`/exam-sessions/${targetExam.id}/start`, {
			...
			if (detailRes.data && Array.isArray(detailRes.data.questions)) {
				detailRes.data.questions = shuffleQuestions(
			...
			try {
				await cacheExamData({
					...
					duration_minutes:
						targetExam.duration_minutes ||
						detailRes.data.duration_minutes ||
						90,
				...
				try {
					await api.post(`/exams/${targetExam.id}/cache-status`);
				} catch (e) {}
			} catch (cacheErr) {
			...
			if (!e.response) {
				...
				if (cachedExam) {
					setTokenError(
						'Server tidak dapat dijangkau. Ketuk "Verifikasi" lagi untuk mode offline.',
					);
				} else {
					setTokenError(
						"Server tidak dapat dijangkau dan soal belum ter-cache.",
					);
				}
			} else {
	...
	const viewResult = async (exam) => {
		try {
			...
			alert("Gagal mengambil hasil ujian");
		}
	};
	...
	const saveAnswer = async (questionId, answer) => {
		...
		if (navigator.onLine) {
			try {
				await api.post(`/exam-sessions/${examId}/answer`, {
				...
				if (
					...
					e.response.data.blocked
				) {
					alert(
						"Anda telah diblokir dari ujian ini. Jawaban tidak dapat disimpan lagi.",
					);
					if (timerRef.current) clearInterval(timerRef.current);
					if (document.exitFullscreen && document.fullscreenElement) {
	...
	const handleSubmit = async (examId, auto = false, isViolation = false) => {
		...
		if (
			!auto &&
			!window.confirm(
				"Yakin ingin mengumpulkan ujian? Jawaban tidak dapat diubah setelah dikumpulkan.",
			)
		)
			return;
		if (timerRef.current) clearInterval(timerRef.current);
		...
		if (!navigator.onLine || offlineMode) {
			try {
				...
				alert(
					"Ujian dikumpulkan secara offline. Jawaban Anda tersimpan aman di perangkat dan akan otomatis dikirim ke server saat koneksi kembali.",
				);
				setView("list");
				fetchExams();
			} catch (e) {
		...
		try {
			...
			try {
				await api.delete(`/exams/${eid}/cache-status`);
			} catch (e) {}
			...
			if (!e.response) {
				...
				alert(
					"Server tidak dapat dijangkau. Jawaban disimpan di perangkat dan akan otomatis dikirim saat koneksi kembali.",
				);
				setView("list");
			} else {
	...
	if (view === "result" && result) {
		...
		const earned = resAnswers.reduce(
			(a, b) =>
				a +
				(b.question_type === "essay"
					? b.essay_score || 0
					: b.points_earned || 0),
			0,
		);
		const essayPending = resAnswers.filter(
			(a) => a.question_type === "essay" && !a.graded_by_dosen,
		).length;
			...
			<div className="animate-fade-in">
					...
					<div className="card-body p-4 text-center">
						<div
							style={{
								fontSize: "3rem",
								fontWeight: 900,
								color: session.total_score >= 75 ? "#16a34a" : "#dc2626",
							}}
						>
							{session.total_score ?? "--"}
						</div>
						...
						{essayPending > 0 && (
							<div className="alert alert-warning mt-3 mb-0 text-start">
								...
								<strong>{essayPending} soal essay</strong> belum dinilai oleh
								dosen. Skor mungkin berubah.
							</div>
						)}
					</div>
				</div>
				...
				{resAnswers.map((a, i) => {
					...
					return (
						<div
							key={a.id}
							className={`card shadow-sm border-0 rounded-4 mb-3 border-start border-4 ${a.question_type === "essay" ? "border-info" : a.is_correct ? "border-success" : "border-danger"}`}
						>
							<div className="card-body p-4">
								<div className="d-flex gap-2 align-items-center mb-2">
									<span className="badge bg-secondary">#{i + 1}</span>
									<span className={`badge bg-${typeBg[a.question_type]}`}>
									...
									{a.question_type === "essay" && (
										<span
											className={`badge ms-auto ${a.graded_by_dosen ? "bg-success" : "bg-warning text-dark"}`}
										>
											{a.graded_by_dosen
												? `Dinilai: ${a.essay_score} poin`
												: "Belum Dinilai"}
										</span>
									)}
								</div>
		...
		return (
			...
			>
				<div className="container" style={{ maxWidth: "1200px" }}>
					...
					{offlineMode && (
						<div
							className="alert mb-3 py-2 px-3 d-flex align-items-center gap-2 border-0 rounded-3 shadow-sm"
							style={{
						...
						>
							...
							<div>
								<strong>Mode Offline</strong> — Soal dimuat dari cache
								perangkat. Jawaban tersimpan lokal dan akan otomatis dikirim ke
					...
					{!isOnline && !offlineMode && (
						<div
							className="alert mb-3 py-2 px-3 d-flex align-items-center gap-2 border-0 rounded-3 shadow-sm animate-fade-in"
							style={{
						...
						>
							...
							<div>
								<strong>Koneksi terputus</strong> — Jawaban Anda tetap tersimpan
								di perangkat ini. Akan otomatis dikirim ke server saat koneksi
								kembali.
							</div>
						</div>
					)}
						...
						<div className="card-body p-4 text-white">
							<div className="d-flex justify-content-between align-items-center">
								<div>
									...
									<div className="d-flex align-items-center gap-2 mt-1">
										...
										{offlineMode && (
											...
											>
												<HardDrive size={10} className="me-1" />
												OFFLINE
											</span>
										)}
									</div>
								...
								<div className="text-center">
									<div
										className={`fw-bold fs-3 ${isLowTime ? "text-warning" : ""}`}
					...
					<div className="row g-4">
								...
								<div className="card-body p-3">
									...
									<div className="d-flex flex-wrap gap-1">
										{questions.map((_, i) => (
											<button
												key={i}
												className={`btn btn-sm fw-bold ${currentQ === i ? "btn-primary" : answers[questions[i]?.id] ? "btn-success" : "btn-outline-secondary"}`}
									...
									<div className="mt-3 d-flex flex-column gap-1">
										<small>
											<span className="badge bg-primary me-1">■</span> Sedang
											Dikerjakan
										</small>
										<small>
											<span className="badge bg-success me-1">■</span> Sudah
											Dijawab
										</small>
										<small>
											<span className="badge bg-secondary me-1">■</span> Belum
											Dijawab
										</small>
									</div>
								</div>
						...
						<div className="col-md-9" key={q ? q.id : "empty"}>
							{questions.length === 0 ? (
								<div
									className="card shadow-sm border-0 rounded-4 h-100 d-flex flex-column align-items-center justify-content-center p-5 text-center"
								...
								>
									...
									<p className="text-muted">
										Dosen belum memasukkan soal ke dalam ujian ini. Silakan
										hubungi dosen Anda.
									</p>
								</div>
							) : (
									...
									<div className="card shadow-sm border-0 rounded-4">
										<div className="card-body p-4">
											<div className="d-flex gap-2 align-items-center mb-3">
												...
												<span className={`badge bg-${typeBg[q.question_type]}`}>
											...
											{q.question_type === "pg" && q.shuffledOptions && (
												<div className="d-flex flex-column gap-2">
													{q.shuffledOptions.map((optObj, oi) => {
														...
														const selected =
															answers[q.id] === optObj.originalLetter;
														return (
															<button
																key={optObj.originalLetter}
																className={`btn text-start rounded-3 fw-semibold border ${selected ? "btn-primary border-primary" : "btn-light border-secondary-subtle"}`}
																style={{ padding: "12px 16px" }}
																onClick={() =>
																	saveAnswer(q.id, optObj.originalLetter)
																}
															>
																<span
																	className={`me-3 badge ${selected ? "bg-white text-primary" : "bg-secondary-subtle text-dark"}`}
																>
																	{displayLetter}
																</span>
																{optObj.text}
											...
											{q.question_type === "true_false" && (
												<div className="d-flex gap-3">
													{[
														...
														<button
															key={val}
															className={`btn rounded-3 fw-bold px-5 py-3 ${answers[q.id] === val ? "btn-primary" : "btn-outline-secondary"}`}
											...
											{q.question_type === "essay" && (
												<textarea
													...
													onPaste={(e) => {
														e.preventDefault();
														alert(
															"Tindakan paste (tempel) dinonaktifkan untuk menjaga kejujuran akademik.",
														);
													}}
		...
		<div className="animate-fade-in">
			...
			{!isOnline && (
				...
				>
					<WifiOff size={18} style={{ flexShrink: 0 }} />
					<div>
						<strong>Mode Offline</strong> — Menampilkan ujian yang tersimpan di
						perangkat. Anda tetap bisa mengerjakan ujian yang sudah di-cache.
					</div>
				</div>
			)}
			{loading ? (
				<div className="text-center py-5 text-muted">Memuat...</div>
			) : exams.length === 0 ? (
				<div className="text-center text-muted py-5">
					<ClipboardList size={48} className="mb-3 opacity-50" />
					<h5>Belum ada ujian tersedia</h5>
				</div>
			) : (
				<div className="row g-3">
					{exams.map((exam) => {
						...
						const hasDeferredSubmit = !!localStorage.getItem(
							"siakad_deferred_submit_" + exam.id,
						);
						...
						const cachedQCount =
							isCached && cachedExamData.questions
								? cachedExamData.questions.length
								: 0;
									...
									<div className="card-body p-4">
										<div className="d-flex justify-content-between align-items-start mb-3">
											<div className="d-flex gap-1 flex-wrap">
												<span
													className={`badge ${exam.type === "UTS" ? "bg-warning text-dark" : "bg-danger"}`}
												>
													{exam.type}
												</span>
												{isCached && (
													...
													>
														<HardDrive size={10} className="me-1" />
														Tersimpan Offline ({cachedQCount} Soal)
													</span>
												)}
											</div>
						...
						<div className="modal-dialog modal-dialog-centered modal-sm">
							<div className="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
								<div className="modal-header border-0 bg-primary text-white pb-3 pt-4 px-4 d-flex flex-column align-items-center">
									<div
										className="bg-white text-primary rounded-circle d-flex align-items-center justify-content-center mb-3 shadow-sm"
										style={{ width: 50, height: 50 }}
									>
										<AlertCircle size={24} />
									</div>
									<h5 className="modal-title fw-bold text-center w-100 mb-0">
										Verifikasi Token
									</h5>
								</div>
								<div className="modal-body p-4 bg-light">
									<form onSubmit={handleStartExam}>
										<p className="text-center text-muted small mb-3">
											Masukkan token ujian untuk{" "}
											{targetExam?.session_id ? "melanjutkan" : "memulai"}{" "}
											<strong className="text-dark">{targetExam?.title}</strong>
										</p>
										<div className="alert alert-danger border-0 bg-danger bg-opacity-10 text-dark small px-3 py-2 mb-3 rounded-3 shadow-sm d-flex align-items-start gap-2 text-start">
											...
											/>
											<div>
												<strong>PERHATIAN! SISTEM ANTI-CURANG AKTIF.</strong>
												<br />
												Ujian akan <b>dihentikan paksa (Skor 0)</b> jika
												terdeteksi keluar layar, membuka aplikasi/tab lain, atau
												Copy-Paste jawaban AI.
											</div>
										</div>
										{tokenError && tokenError.includes("AKSES DITOLAK") ? (
											<div className="alert alert-danger fw-bold text-center border-2 border-danger shadow-sm mb-3 px-2 py-3">
												{tokenError}
											</div>
										) : null}
										<div className="mb-3">
										...
										{!isStarting && (
											<button
												type="button"
												className="btn btn-link text-muted text-decoration-none w-100 mt-2 small"
							...
							<div className="modal-content border-0 shadow">
								<div className="modal-header border-0 pb-0">
									<h5 className="modal-title fw-bold">
										Kisi-Kisi Ujian: {activeKisiExam.title}
									</h5>
									<button
										type="button"
										className="btn-close"
								...
								<div className="modal-body">
									<div className="alert alert-info border-0 bg-info bg-opacity-10 text-info mb-3">
										<strong>Pelajari kisi-kisi berikut</strong> sebelum mulai
										mengerjakan ujian.
									</div>
									<div
										className="table-responsive"
										dangerouslySetInnerHTML={{
											__html: activeKisiExam.kisi_kisi,
										}}
									/>
								</div>
