...
import { ClipboardList, CheckCircle, AlertTriangle, Send, Loader } from 'lucide-react';
...
export default function MahasiswaEDOM() {
  ...
  if (loading) return <div className="text-center py-5"><Loader className="spin" size={32} /> Memuat data EDOM...</div>;

  if (selectedSchedule) {
    return (
      <div className="animate-fade-in">
        <button className="btn btn-outline-secondary mb-3" onClick={() => setSelectedSchedule(null)}>← Kembali</button>
        <div className="card shadow-sm border-0 rounded-4">
          <div className="card-header bg-white border-bottom p-4">
            <h5 className="fw-bold mb-1">Evaluasi Dosen: {selectedSchedule.dosen_name}</h5>
            <p className="text-muted small mb-0">{selectedSchedule.course_name} ({selectedSchedule.class_name})</p>
          </div>
          <div className="card-body p-4">
            <div className="alert alert-info">
              Penilaian ini bersifat <strong>anonim</strong> dan sangat penting untuk evaluasi kinerja dosen dan institusi.
              Beri nilai 1 (Sangat Kurang) hingga 5 (Sangat Baik).
            </div>

            {questions.map((q, idx) => (
              <div key={q.id} className="mb-4 border-bottom pb-3">
                <p className="fw-semibold mb-2">{idx + 1}. {q.question_text}</p>
            ...
            <div className="mb-4">
              <label className="fw-semibold form-label">Kritik & Saran (Opsional)</label>
              <textarea 
            ...
            >
              {submitting ? <Loader className="spin" size={20} /> : <Send size={20} className="me-2"/>} 
              Kirim Evaluasi
            </button>
          </div>
        </div>
  ...
  const allCompleted = schedules.length > 0 && schedules.every(s => s.is_filled > 0);

  return (
    <div className="animate-fade-in">
      <div className="mb-4">
        <h3 className="fw-bold mb-0 text-dark"><ClipboardList size={28} className="me-2 text-primary" /> Evaluasi Dosen (EDOM)</h3>
      ...
      {allCompleted ? (
         <div className="alert alert-success d-flex align-items-center rounded-4 p-4 shadow-sm">
           ...
           <div>
             ...
             <p className="mb-0">Anda telah menyelesaikan seluruh Evaluasi Dosen untuk semester ini. Akses KHS Anda telah terbuka.</p>
           </div>
         </div>
      ) : (
        <div className="alert alert-warning d-flex align-items-center rounded-4 p-4 shadow-sm mb-4">
          ...
          <div>
            ...
            <p className="mb-0">Anda harus mengisi EDOM untuk semua kelas aktif Anda agar dapat melihat Kartu Hasil Studi (KHS).</p>
          </div>
        </div>
      )}
            ...
            <div className="card shadow-sm border-0 rounded-4 h-100">
              <div className="card-body p-4 d-flex justify-content-between align-items-center">
                <div>
                  ...
                  <span className="badge bg-light text-dark border">{s.class_name}</span>
                </div>
                <div>
                  {s.is_filled > 0 ? (
                    ...
                    <button className="btn btn-primary btn-sm rounded-pill px-3" onClick={() => handleSelect(s)}>
                      Isi Evaluasi
                    </button>
                  )}
                </div>
