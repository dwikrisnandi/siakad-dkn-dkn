...
export default function AdminFCMTokens() {
  ...
  const grouped = tokens.reduce((acc, t) => {
    const key = t.user_id;
    if (!acc[key]) {
      acc[key] = { name: t.name, nidn_nim: t.nidn_nim, role: t.role, tokens: [] };
    }
  ...
  return (
    <div className="animate-fade-in">
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <button className="btn btn-outline-primary btn-sm" onClick={fetchTokens} disabled={loading}>
      ...
      <div className="row mb-3">
        <div className="col-md-4">
          <div className="card border-0 shadow-sm rounded-4">
            <div className="card-body text-center">
              <h4 className="fw-bold text-primary">{Object.keys(grouped).length}</h4>
        ...
        <div className="col-md-4">
          <div className="card border-0 shadow-sm rounded-4">
            <div className="card-body text-center">
              <h4 className="fw-bold text-warning">{tokens.length > 0 ? (tokens.length / Object.keys(grouped).length).toFixed(1) : 0}</h4>
      ...
      {loading ? (
        ...
        <div className="alert alert-info">Belum ada perangkat yang terdaftar.</div>
      ) : (
        Object.entries(grouped).map(([userId, user]) => (
          <div className="card border-0 shadow-sm rounded-4 mb-3" key={userId}>
            <div className="card-header bg-white border-0 d-flex justify-content-between align-items-center py-3 px-4">
              ...
              <span className="badge bg-secondary">{user.tokens.length} perangkat</span>
            </div>
                    ...
                    <tr key={t.id}>
                      ...
                      <td>
                        <code style={{ fontSize: '11px', wordBreak: 'break-all' }}>
                          {t.token.substring(0, 40)}...{t.token.substring(t.token.length - 10)}
                        </code>
                      </td>
                      <td>
                        <small className="text-muted">
                          {t.last_used_at ? new Date(t.last_used_at).toLocaleString('id-ID') : '-'}
                        </small>
                      </td>
                      <td>
                        <button
