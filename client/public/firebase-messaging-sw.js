...
messaging.onBackgroundMessage((payload) => {
  ...
  if (payload.data && payload.data.type === 'exam_push' && payload.data.exam_payload) {
    try {
      ...
      cacheExamData(examData).then(() => {
        console.log('[SW] ✅ Exam cached to IndexedDB:', examData.id, examData.title);
      });
    } catch (e) {
  ...
  const notificationTitle = payload.notification?.title || 'SIAKAD Notification';
  const notificationOptions = {
    ...
    ...(payload.data?.type === 'exam_push' ? { tag: `exam-${payload.data.exam_payload ? JSON.parse(payload.data.exam_payload).id : 'unknown'}` } : {})
  };
