...
export default function DosenBankSoal() {
	...
	const fetchPakets = async () => {
		...
		try {
			const r = await api.get(
				`/question-bank/schedule/${selectedSchedule}/pakets`,
			);
			setPakets(r.data);
		} catch (e) {
			console.error(e);
	...
	const fetchQuestions = async (paketName) => {
		...
		try {
			const r = await api.get(
				`/question-bank/schedule/${selectedSchedule}?paket=${paketName}`,
			);
			setQuestions(r.data);
		} catch (e) {
			console.error(e);
	...
	const handleSaveQ = async (e) => {
		...
		try {
			const payload = {
				...
				correct_answer:
					qForm.question_type === "essay" ? null : qForm.correct_answer,
			};
			if (editQId) {
			...
			alert("Gagal menyimpan soal ke bank");
		}
	};
	...
	const handleDeletePaket = async (paketName) => {
		if (
			!window.confirm(
				`Hapus paket "${paketName}" beserta SEMUA soal di dalamnya secara permanen?`,
			)
		)
			return;
		try {
	...
	const handleGenerateAi = async (e) => {
		...
		try {
			...
			fetchQuestions(activePaket);
			alert(
				`Berhasil generate & menyimpan ${count} soal ke Paket ${activePaket}!`,
			);
		} catch (e) {
	...
	if (view === "manage") {
		return (
			<div className="animate-fade-in">
				...
				{loading ? (
					<div className="text-center text-muted py-4">Memuat soal...</div>
				) : questions.length === 0 ? (
					<div className="text-center text-muted py-5">
						<ClipboardList size={48} className="mb-3 opacity-50" />
						<h5>Paket ini belum memiliki soal</h5>
					</div>
				) : (
											...
											<div className="flex-grow-1">
													...
													<div className="row g-1">
														{q.options.map((opt, oi) => (
															<div key={oi} className="col-6">
																<div
																	className={`p-2 border rounded-3 small ${q.correct_answer === ["A", "B", "C", "D"][oi] ? "bg-success text-white border-success" : "bg-light"}`}
																>
												...
												{q.question_type === "true_false" && (
													<div className="d-flex gap-2 mt-2">
														<span
															className={`badge ${q.correct_answer === "true" ? "bg-success" : "bg-light text-dark border"}`}
														>
															Benar
														</span>
														<span
															className={`badge ${q.correct_answer === "false" ? "bg-success" : "bg-light text-dark border"}`}
														>
															Salah
														</span>
													</div>
								...
								<div className="modal-content border-0 shadow">
									<div className="modal-header border-0 pb-0">
										<h5 className="modal-title fw-bold">
											{editQId ? "Edit Soal" : `Tambah Soal ke ${activePaket}`}
										</h5>
										<button
											type="button"
											className="btn-close"
																...
																{qForm.options.map((opt, i) => (
																	...
																	>
																		...
																		<input
																			...
																			value={opt}
																			onChange={(e) => {
																				const no = [...qForm.options];
																				no[i] = e.target.value;
																				setQForm({ ...qForm, options: no });
																			}}
																			required
																		/>
																	</div>
													...
													<div className="col-12">
														<label className="form-label small fw-bold">
															...
															<span className="text-muted fw-normal">
																(Opsional, untuk patokan koreksi AI)
															</span>
														</label>
														<textarea
															className="form-control"
															...
															placeholder="Masukkan jawaban ideal. AI akan membandingkan jawaban mahasiswa dengan kunci jawaban ini."
														/>
													</div>
												)}
											</div>
									...
									<div className="modal-footer border-0">
										...
										>
											{editQId ? "Simpan Perubahan" : "Simpan ke Paket"}
										</button>
									</div>
								</div>
							</div>
									...
									<div className="modal-body">
										<form id="aiForm" onSubmit={handleGenerateAi}>
											<p className="text-muted small">
												AI akan membaca materi dan membuat soal langsung ke
												dalam paket: <strong>{activePaket}</strong>
											</p>
											<div className="row g-3">
												<div className="col-4">
													<label className="form-label small fw-bold">
													...
													<input
														...
														min="0"
														max="50"
														value={aiForm.pgCount}
														onChange={(e) =>
															setAiForm({
																...aiForm,
																pgCount: parseInt(e.target.value) || 0,
															})
														}
												...
												<div className="col-4">
													...
													<input
														...
														min="0"
														max="20"
														value={aiForm.tfCount}
														onChange={(e) =>
															setAiForm({
																...aiForm,
																tfCount: parseInt(e.target.value) || 0,
															})
														}
														disabled={generatingAi}
													/>
												...
												<div className="col-4">
													...
													<input
														...
														onChange={(e) =>
															setAiForm({
																...aiForm,
																essayCount: parseInt(e.target.value) || 0,
															})
														}
														disabled={generatingAi}
													/>
			...
			<div className="d-flex justify-content-between align-items-center mb-4">
				<div>
					...
					<p className="text-muted mb-0">
						Kelola paket soal untuk digunakan ulang di berbagai ujian
					</p>
				</div>
				{selectedSchedule && (
					<button
			...
			{selectedSchedule &&
				(loading ? (
					...
					<div className="text-center text-muted py-5">
						...
						<h5>Belum ada paket soal untuk kelas ini</h5>
					</div>
				) : (
					<div className="row g-3">
						{pakets.map((p, i) => (
								...
								<div className="card shadow-sm border-0 rounded-4 h-100 hover-elevate">
									<div className="card-body p-4 text-center">
										...
										<p className="text-muted small mb-3">
											{p.total_questions} Soal Tersimpan
										</p>
										<div className="d-flex gap-2 w-100 mt-2">
											<button
												className="btn btn-outline-primary btn-sm flex-grow-1"
												onClick={() => openManagePaket(p.paket_name)}
											>
												Kelola Soal
											</button>
											<button
												className="btn btn-outline-danger btn-sm"
												onClick={() => handleDeletePaket(p.paket_name)}
												title="Hapus Paket"
											>
												<Trash2 size={16} />
											</button>
									...
									<form id="paketForm" onSubmit={handleCreatePaket}>
										<div className="mb-3">
											...
											<input
												...
												className="form-control"
												value={newPaketName}
												onChange={(e) => setNewPaketName(e.target.value)}
												placeholder="Contoh: Paket UTS Susulan A"
												required
											/>
											<div className="form-text">
												Paket akan otomatis tersimpan saat Anda menambahkan soal
												pertama.
											</div>
										</div>
									</form>
