...
export default function DosenKehadiran() {
	...
	useEffect(() => {
		...
		const selectedSched = schedules.find(
			(s) => s.id === parseInt(selectedSchedule),
		);
		...
		const fetchStudentsAndAttendance = async () => {
			...
			try {
				...
				if (
					selectedSched.class_ids_array &&
					selectedSched.class_ids_array.length > 0
				) {
				...
				const resAttendance = await api.get(
					`/attendance/${selectedSchedule}?meeting_number=${selectedMeeting}`,
				);
				const existingRecords = resAttendance.data;

				// Fetch Existing Note for this meeting
				try {
					const resNote = await api.get(
						`/attendance-note/${selectedSchedule}?meeting_number=${selectedMeeting}`,
					);
					...
					<div className="card-body p-0">
												...
												<td className="pe-4">
													<select
														className={`form-select form-select-sm fw-bold w-auto ${
															attendanceData[m.id] === "Hadir"
																? "bg-success-subtle text-success border-success-subtle"
																: attendanceData[m.id] === "Izin/Sakit"
						...
						<div className="p-4 border-top bg-light">
							<label className="form-label fw-bold text-muted small">
								Resume / Catatan Hasil Pertemuan
							</label>
							<textarea
						...
						<div className="p-4 border-top d-flex justify-content-between align-items-center bg-white rounded-bottom-4">
							<div>
								{saveStatus && (
									<span
										className={`small fw-bold ${saveStatus.includes("Gagal") ? "text-danger" : "text-success"}`}
									>
										{saveStatus}
									</span>
								)}
