...
import { Printer, GraduationCap, Loader, CheckCircle } from 'lucide-react';
...
export default function MahasiswaTranskrip() {
  ...
  if (loading) return <div className="text-center py-5"><Loader className="spin" size={32} /> Memuat data Transkrip...</div>;
  if (!data) return <div className="alert alert-danger">Gagal memuat transkrip.</div>;

  return (
    <div className="animate-fade-in">
      <div className="d-flex justify-content-between align-items-center mb-4 d-print-none">
        <div>
          <h3 className="fw-bold mb-0 text-dark"><GraduationCap size={28} className="me-2 text-primary" /> Transkrip Akademik</h3>
          <p className="text-muted small mb-0">Rangkuman nilai kumulatif mahasiswa</p>
        </div>
        <button className="btn btn-primary rounded-pill px-4 fw-bold" onClick={handlePrint}>
