			...
			<div className="d-flex justify-content-between align-items-center mb-4">
				<div>
					<h3 className="fw-bold mb-0 text-dark">
						<BookOpen size={28} className="me-2 text-primary" /> Master
						Kurikulum
					</h3>
					<p className="text-muted small mb-0">
						Kelola tahun berlaku kurikulum untuk tiap program studi
					</p>
				</div>
				<button
								...
								filtered.map((c) => (
									<tr key={c.id}>
										<td className="px-4 fw-semibold">
											{c.nama_prodi || "Tidak Diketahui"}
										</td>
										<td>{c.tahun_berlaku}</td>
										<td>
											{c.status_aktif ? (
												<span className="badge bg-success-subtle text-success border border-success-subtle">
													<CheckCircle size={12} className="me-1" /> Aktif
												</span>
											) : (
												<span className="badge bg-secondary-subtle text-secondary border border-secondary-subtle">
													<XCircle size={12} className="me-1" /> Nonaktif
												</span>
											)}
										</td>
							...
							>
								<div className="modal-header border-bottom-0 pb-0">
									<h5 className="modal-title fw-bold">
										{formData.id ? "Edit Kurikulum" : "Tambah Kurikulum Baru"}
									</h5>
									<button
										type="button"
										className="btn-close"
