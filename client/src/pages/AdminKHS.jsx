import { ArrowLeft, Award, Search, UserCircle } from "lucide-react";
...
export default function AdminKHS() {
	...
	const filteredMhs = mahasiswas.filter(
		(m) =>
				...
				<div>
					...
					{selectedMhs && (
						<small className="text-muted">
							{selectedMhs.name} ({selectedMhs.nidn_nim})
						</small>
					)}
				</div>
			</div>
				...
				<>
					{loading ? (
						...
						<div className="text-center text-muted py-5 card border-0 shadow-sm rounded-4">
							...
							<p className="mb-0">
								Mahasiswa ini belum terdaftar di kelas manapun atau belum ada
												...
												return (
													<tr key={idx}>
														<td className="ps-4 py-3">
															...
															<small className="text-muted">
																{item.course_code} - Sem {item.semester}
															</small>
														</td>
														<td>{item.sks}</td>
														<td>
															<small>{item.dosen_name}</small>
														</td>
														...
														<td className="text-center pe-4">
															<span
																className={`badge bg-${grade.color} fs-6 px-3`}
															>
																{grade.letter}
															</span>
														</td>
										...
										<tfoot className="table-light fw-bold">
											<tr>
												<td colSpan={7} className="text-end py-3">
													Indeks Prestasi Semester (IPS) Sementara :
												</td>
												<td
													colSpan={2}
													className="text-center py-3 fs-5 text-primary"
												>
													{(
														khsData.reduce(
															(acc, curr) =>
														...
														Math.max(
															khsData.reduce((acc, curr) => acc + curr.sks, 0),
															1,
														)
													).toFixed(2)}
												</td>
