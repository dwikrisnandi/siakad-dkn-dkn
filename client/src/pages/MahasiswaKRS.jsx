...
export default function MahasiswaKRS() {
	...
	const fetchData = async () => {
		...
		try {
			...
			if (
				resStatus.data.is_lunas &&
				(!resStatus.data.krs ||
					resStatus.data.krs.status_approval !== "Approved")
			) {
				...
				if (resStatus.data.krs_items) {
					setSelectedIds(
						resStatus.data.krs_items.map((item) => item.schedule_id),
					);
				}
			}
		} catch (err) {
	...
	const handleToggle = (schedId) => {
		...
		setSelectedIds((prev) =>
			prev.includes(schedId)
				? prev.filter((id) => id !== schedId)
				: [...prev, schedId],
		);
	};
	...
	const handleSubmit = async () => {
		...
		try {
			...
			// GTM: Lacak Event Pengisian KRS
			window.dataLayer = window.dataLayer || [];
			window.dataLayer.push({ event: "krs_submission", status: "success" });

			alert("KRS berhasil diajukan!");
			fetchData();
		} catch (err) {
			// GTM: Lacak kegagalan pengisian KRS
			window.dataLayer = window.dataLayer || [];
			window.dataLayer.push({ event: "krs_submission", status: "failed" });

			alert(
				err.response?.data?.error || "Terjadi kesalahan saat mengajukan KRS",
			);
		} finally {
			setSubmitting(false);
		}
	...
	if (loading)
		return (
			<div className="text-center py-5">
				<Loader className="spin" size={32} /> Memuat data KRS...
			</div>
		);
	if (!status)
		return (
			<div className="alert alert-danger">Gagal memuat status sistem.</div>
		);

	if (!status.academic_year) {
		return (
			<div className="alert alert-warning">
				Tahun akademik saat ini belum aktif. Silakan hubungi admin.
			</div>
		);
	}
			...
			<div className="card border-danger shadow-sm rounded-4 overflow-hidden animate-fade-in">
				<div className="card-body p-5 text-center">
					...
					<p className="text-muted">
						Status keuangan Anda untuk tahun akademik{" "}
						<strong>{status.academic_year.name}</strong> belum lunas. <br />
						Selesaikan tagihan administrasi Anda untuk dapat mengisi Kartu
						Rencana Studi (KRS).
					</p>
				</div>
			</div>
	...
	if (!status.program_id) {
		return (
			<div className="alert alert-warning">
				Anda belum diplot ke Program Studi manapun. Silakan hubungi akademik.
			</div>
		);
	}
	if (!status.dpa_id) {
		return (
			<div className="alert alert-warning">
				Anda belum memiliki Dosen Pembimbing Akademik (DPA). Silakan hubungi
				akademik.
			</div>
		);
	}
		...
		<div className="animate-fade-in">
			<div className="mb-4">
				<h3 className="fw-bold mb-0 text-dark">
					<BookOpen size={28} className="me-2 text-primary" /> Pengisian KRS
				</h3>
				<p className="text-muted small mb-0">
					Tahun Akademik {status.academic_year.name}
				</p>
			</div>

			{krsState && (
				<div
					className={`alert alert-${isApproved ? "success" : isPending ? "warning" : "danger"} d-flex align-items-center mb-4`}
				>
					{isApproved ? (
					...
					<div className="table-responsive">
						<table className="table table-hover align-middle mb-0">
							...
							<tfoot className="table-light fw-bold">
								<tr>
									<td colSpan="2" className="text-end px-4">
										Total SKS:
									</td>
									<td colSpan="4">
										{status.krs_items.reduce((s, i) => s + i.sks, 0)} SKS
									</td>
								</tr>
							</tfoot>
						</table>
								...
								{availableSchedules.length === 0 ? (
									<tr>
										<td colSpan="7" className="text-center py-4 text-muted">
											Belum ada jadwal yang dibuka untuk program studi Anda.
										</td>
									</tr>
								) : (
									availableSchedules.map((s) => {
										...
										return (
											<tr
												key={s.id}
												className={isChecked ? "table-primary-subtle" : ""}
											>
												<td className="ps-4">
													<input
														type="checkbox"
					...
					{!isPending && availableSchedules.length > 0 && (
						<div className="card-footer bg-white border-top p-4 d-flex justify-content-between align-items-center">
							<div className="text-muted small">
								Pastikan Anda telah berkonsultasi dengan Dosen PA sebelum
								mengajukan.
							</div>
							<button
