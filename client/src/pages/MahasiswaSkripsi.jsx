...
export default function MahasiswaSkripsi() {
	...
	const handleAddLogbook = async (e) => {
		e.preventDefault();
		if (!lbDate || !lbActivity)
			return alert("Tanggal dan Aktivitas wajib diisi!");
		setSubmitting(true);
		try {
			await api.post(`/skripsi/${data.id}/logbooks`, {
				date: lbDate,
				activity: lbActivity,
				note: lbNote,
			});
	...
	if (loading)
		return (
			<div className="text-center py-5">
				<Loader className="spin" size={32} /> Memuat data Skripsi...
			</div>
		);
		...
		return (
			<div className="animate-fade-in">
				<h3 className="fw-bold mb-4 text-dark">
					<BookOpen size={28} className="me-2 text-primary" /> Pengajuan Judul
					Skripsi
				</h3>
				<div className="card shadow-sm border-0 rounded-4">
					<div className="card-body p-4">
						<div className="alert alert-info">
							Silakan ajukan usulan judul Skripsi/Tugas Akhir Anda. Anda dapat
							mengajukan maksimal 3 opsi judul.
						</div>
						<form onSubmit={handleSubmit}>
							<div className="mb-3">
								<label className="form-label fw-bold">
									Usulan Judul 1 (Wajib)
								</label>
								<textarea
							...
							>
								{submitting ? (
									...
									<>
										<Send size={18} className="me-2" /> Ajukan Judul
									</>
								)}
							</button>
						</form>
	...
	return (
		<div className="animate-fade-in">
			<h3 className="fw-bold mb-4 text-dark">
				<BookOpen size={28} className="me-2 text-primary" /> Ruang Skripsi /
				Tugas Akhir
			</h3>
			...
			{isPending && (
				<div className="alert alert-warning d-flex align-items-center mb-4 rounded-4 p-4 shadow-sm">
					...
					<div>
						...
						<p className="mb-0">
							Usulan judul Anda sedang ditinjau oleh Ketua Program Studi.
						</p>
					</div>
				</div>
			)}
				...
				<div className="col-md-4">
					<div className="card shadow-sm border-0 rounded-4 mb-4">
						<div className="card-header bg-white border-bottom p-3">
							<h6 className="fw-bold mb-0">Informasi Pembimbing</h6>
						</div>
							...
							{data.pembimbing_1_name ? (
								<>
									...
									<p className="fw-bold mb-0">
										{data.pembimbing_2_name || "-"}
									</p>
								</>
							) : (
								<p className="text-muted small fst-italic mb-0">
									Belum ada pembimbing yang ditunjuk.
								</p>
							)}
						</div>
					</div>
									...
									<form onSubmit={handleAddLogbook}>
										<div className="row g-2 mb-2">
											...
											<div className="col-md-8">
												<input
													type="text"
													className="form-control form-control-sm"
													placeholder="Aktivitas bimbingan..."
													value={lbActivity}
													onChange={(e) => setLbActivity(e.target.value)}
													required
												/>
											</div>
										</div>
										<div className="mb-2">
											<textarea
												className="form-control form-control-sm"
												rows="2"
												placeholder="Catatan/Revisi dari dosen (opsional)..."
												value={lbNote}
									...
									logbooks.map((lb) => (
										<div className="list-group-item p-3" key={lb.id}>
											<div className="d-flex justify-content-between align-items-start mb-1">
												...
												<span
													className={`badge ${lb.status_validation === "Approved" ? "bg-success" : lb.status_validation === "Rejected" ? "bg-danger" : "bg-warning text-dark"}`}
												>
