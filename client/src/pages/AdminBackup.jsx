...
export default function AdminBackup() {
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      setError('Gagal membuat file backup. Pastikan server memiliki akses ke pg_dump.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="card-body p-4">
          <div className="d-flex align-items-center mb-4">
            ...
            <div>
              ...
              <p className="text-muted small mb-0">Download seluruh data seperti Pengguna, Kelas, Jadwal, Nilai, dll.</p>
            </div>
          </div>
          ...
          {success && (
            <div className="alert alert-success d-flex align-items-center mb-4 small py-2">
              ...
              <div>Database berhasil di-backup. Periksa folder Download Anda.</div>
            </div>
          )}

          <div className="bg-light p-3 rounded-3 mb-4 small text-muted border">
            <b>Catatan:</b><br/>
            Proses ini akan mengekspor skema dan data tabel (PostgreSQL) ke dalam sebuah file <code>.sql</code>. Gambar tugas yang tersimpan di dalam folder server tidak termasuk di dalam file ini.
          </div>
          ...
          >
            {loading ? (
              <><span className="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span> Memproses Backup...</>
            ) : (
